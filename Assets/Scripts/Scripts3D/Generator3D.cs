...
public class Generator3D : MonoBehaviour
{
    ...
    class Room
    {
        ...
        public List<GrimmGen.Tile> tiles;

        public Room(Vector3Int location, Vector3Int size) => bounds = new BoundsInt(location, size);

        public List<GrimmGen.Tile> GetTilesInRoom()
        {
    ...
    void PlaceRooms()
    {
        ...
        {
            //Get a random location from within the available space
            ...
            //Replace with choosing a room from the list of available rooms

            bool add = true;
            Room newRoom = new(location, roomSize);
            //Buffer ensures no rooms are touching
            //We don't want a buffer
            //sets the buffer location down and to the left of the bottom left corner by one unit.
            //Sets the size as up and to the right of the top right corner by two units.
            //only creates the bu
            Room buffer = new(location + new Vector3Int(-1, 0, -1), roomSize + new Vector3Int(2, 0, 2));
    ...
    void PlaceRoom(Vector3Int location, Vector3Int size) => PlaceCube(location, size, redMaterial);

    void PlaceHallway(Vector3Int location) => PlaceCube(location, new Vector3Int(1, 1, 1), blueMaterial);

    void PlaceStairs(Vector3Int location) => PlaceCube(location, new Vector3Int(1, 1, 1), greenMaterial);
}
