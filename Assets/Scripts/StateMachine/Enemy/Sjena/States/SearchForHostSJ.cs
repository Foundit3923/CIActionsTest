...
{
    public class SearchForHostState : EnemyControllerState
    {
        private bool _finished;
        public SearchForHostState(EnemyControllerContext context, EnemyControllerStateMachine.EEnemyState estate) : base(context, estate)
        {
        ...
        public override void EnterState()
        {
            ...
            Context.SjenaMonitor(true);
        }
        public override void ExitState() => Context.setLastState(StateKey);

        public override void UpdateState() => SearchForHost();

        void SearchForHost()
        {
            Debug.Log("Sjena is searching for a host");

            //check light level of player (using LightLevelTracker)
            if (Context.lightLevelTripped)
            {
                Context.SetLightLevelTripped(false);
                //if light level is below threshold, transition to attach state
                Debug.Log("Light level is low! Attaching to player.");
                Context.PrerequisiteStateSequence.Add(EnemyControllerStateMachine.EEnemyState.Animation);
                Context.MainStateSequence.Add(EnemyControllerStateMachine.EEnemyState.Attach);
                _finished = true;
            }
            else
            {
                //Otherwise, Sjena keeps searching
                Debug.Log("Light level is high, continuing search.");
            }
        }
