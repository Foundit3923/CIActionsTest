...
namespace LanternHeadStates
{
    public class ResetState : EnemyControllerState
    {
        ...
        private bool isActiveAndOnNavMesh = false;
        ...
        public override void EnterState()
        {
            ...
            _finished = true;
            isActiveAndOnNavMesh = Context.Agent.isActiveAndEnabled && Context.Agent.isOnNavMesh;
            EvalIsReady();
        }
        public override void ExitState()
            //Context.Animator.SetBool("isIdleLH", false);
            => Context.setLastState(StateKey);
        public override void UpdateState()
        {

            if (!isActiveAndOnNavMesh)
            {
                isActiveAndOnNavMesh = Context.Agent.isActiveAndEnabled && Context.Agent.isOnNavMesh;
            }
            else
        ...
        public void EvalIsReady()
        {
            if (isActiveAndOnNavMesh)
            {
                ...
                Context.MainStateSequence.Add(EnemyControllerStateMachine.EEnemyState.Patrol);
                _finished = true;
            }
        }
